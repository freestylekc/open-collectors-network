...
import { latestTime, timeConverter } from './helpers/latestTime';
    ...
    founder1, founder2])
{
    ...
    it('should whitelist the first two investors', async function ()
    {
        ...
        await this.crowdsale.addAddressToWhitelist(otherInvestor, { from: owner });
    });
    ...
    {
        await this.token.transfer(unknownInvestor, 1, { from: investor }).should.be.rejectedWith(EVMRevert);
    });
    ...
    it('should give bonus tokens for amounts over bonus thresholds', async function ()
    {
        ...
        await this.crowdsale.buyTokens(otherInvestor, { value: investmentAmount, from: otherInvestor }).should.be.fulfilled;
    ...
    it('should only allow whitelisted users to participate', async function ()
    {
        ...
        await this.crowdsale.buyTokens(unknownInvestor, { value: ether(1), from: unknownInvestor }).should.be.rejectedWith(EVMRevert);
    });
    ...
    it('should only allow owner to set bonuses', async function ()
    {
        ...
        await this.crowdsale.setBonusThresholds(bonuses, thresholds, { from: owner }).should.be.fulfilled;
        await this.crowdsale.setBonusThresholds(bonuses, thresholds, { from: unknownInvestor }).should.be.rejectedWith(EVMRevert);
    });
    ...
    it('should only allow the owner to whitelist an investor', async function ()
    {
        ...
        await this.crowdsale.addAddressToWhitelist(unknownInvestor, { from: unknownInvestor }).should.be.rejectedWith(EVMRevert);
    })

    it('should allow transfers after unpausing token', async function ()
    {
        await this.token.unpause();
        await this.token.transfer(unknownInvestor, 1, { from: investor }).should.be.fulfilled;
    });
