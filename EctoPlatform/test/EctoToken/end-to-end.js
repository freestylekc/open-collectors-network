 ...
 contract('End-to-End lifetime test', function(accounts) {
    ...
    describe('initial setup', async () => {
      ...
      it('Deploy TokenSale Contract', async () => {
          ...
          console.log('************Initial wallet balance: ', web3.fromWei(web3.eth.getBalance(wallet).toString()))
    ...
    describe('Buying tokens during sale', async () => {
        it('Whitelisted1 buys tokens', async () => {
             ...
             console.log('************Whitelisted1 wallet balance before: ', web3.fromWei(web3.eth.getBalance(whitelisted1).toString()))
             await web3.eth.sendTransaction({ from: whitelisted1, to: sale.address, value: web3.toWei(1, 'ether'), gas:200000 })
             console.log('************Whitelisted1 wallet balance after: ', web3.fromWei(web3.eth.getBalance(whitelisted1).toString()))
             ...
             console.log('************Total wallet balance after whitelist1: ', web3.fromWei(web3.eth.getBalance(wallet).toString()))
             
        })

        it('Whitelisted2 buys tokens in week 3', async () => {
         ...
         await web3.eth.sendTransaction({ from: whitelisted2, to: sale.address, value: web3.toWei(1, 'ether'), gas:200000  })
         ...
         console.log('************Total wallet balance after whitelist2: ', web3.fromWei(web3.eth.getBalance(wallet).toString()))
         
        })

        //it('Nonwhitelisted tries to buy tokens', async () => {
        //     Utils.expectRevert(sale.buyTokens({ from: advisor1, value: web3.toWei(1, 'ether') }))
        //})

        //it('Whitelisted1 tries to transfer to whitelisted2 before finalize', async () => {
        //     Utils.expectRevert(token.transfer(whitelisted2, { from: whitelisted1 }))
        //})
    })
    ...
    describe('Burn unsold tokens', async () => {
         it("Burn and check balances after", async () => {
             const balanceSaleBefore = await token.balanceOf(sale.address) //105MM
    ...
    describe('Token exchange', async () => {
         ...
         it("Final wallet balance", async () => {
            console.log('************Final wallet balance: ', web3.fromWei(web3.eth.getBalance(accounts[15]).toString()))
